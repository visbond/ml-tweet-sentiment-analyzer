# 9414 Assignment 2, DT_sentiment.py
# takes train and test files via commandline, makes predictions using Decision Tree classifier
# uses only the 1000 most common words in the vocabulary, and limits leaf nodes
# to 1% of training set (50 examples here), to avoid excessive branching
...
count = CountVectorizer (tokenizer=cv_tokenizer, max_features = 1000, lowercase = False) #we need to call our custom tokenizer because the default one removes @
                                                                    # max_features = 1000 limits vocabulary & feature length to 1000 most common words
...
clf = tree.DecisionTreeClassifier(min_samples_leaf=.01, criterion='entropy', random_state=0) #first arg is .01 (for 1%), better than 50 
                                                                                 # (which would be hardcoded for just this training set)
